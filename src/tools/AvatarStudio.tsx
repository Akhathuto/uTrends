...
const SpeakingIndicator = ({ isSpeaking }: { isSpeaking: boolean }) => (
    <div className={`relative w-48 h-48 bg-[hsl(var(--secondary))] rounded-full flex items-center justify-center transition-all duration-300 ${isSpeaking ? 'scale-105' : ''}`}>
        <div className={`absolute inset-0 bg-[hsl(var(--primary))] rounded-full transition-opacity duration-300 ${isSpeaking ? 'animate-pulse opacity-50' : 'opacity-0'}`}></div>
        <UserHexagonIcon className="w-24 h-24 text-[hsl(var(--muted-foreground))] z-10" />
    </div>
);
    ...
    return (
        <div className="flex flex-col h-full max-w-4xl mx-auto items-center">
            <div className="text-center mb-8">
                ...
                <p className="text-[hsl(var(--muted-foreground))]">Have a real-time conversation with your AI persona.</p>
            </div>
            ...
            <div className="my-8">
                <button
                    ...
                    className="px-8 py-4 bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] font-semibold rounded-full hover:opacity-90 disabled:opacity-50 transition-colors flex items-center gap-3 text-lg"
                >
            ...
            <div className="w-full flex-grow bg-[hsl(var(--card))] rounded-[var(--radius)] border border-[hsl(var(--border))] p-4 overflow-y-auto">
                <h3 className="font-bold mb-2">Live Transcription</h3>
                {error && <p className="text-red-400">{error}</p>}
                {transcription.length === 0 && !isRecording && !isConnecting && <p className="text-[hsl(var(--muted-foreground))]">Click "Start Conversation" to begin.</p>}
                <div className="space-y-2 text-sm">
                    {transcription.map((turn, index) => (
                        <div key={index}>
                            <p><strong className="text-[hsl(var(--primary))]">You:</strong> {turn.user}</p>
                            <p><strong className="text-green-500">Nolo:</strong> {turn.model}</p>
                        </div>
                    ))}
                </div>
            </div>
